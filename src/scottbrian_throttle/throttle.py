...
from typing import (Any, Callable, cast, Final, NamedTuple, Optional,
                    overload, Protocol, Type, TYPE_CHECKING, TypeVar,
                    Union)
...
class AsyncQSizeNotAllowed(ThrottleError):
    """Throttle exception async_q_size specified when not allowed."""
    pass


class AttemptedShutdownForSyncThrottle(ThrottleError):
    """Throttle exception shutdown not in mode Throttle.MODE_ASYNC."""
    pass
...
class LbThresholdNotAllowed(ThrottleError):
    """Throttle exception lb_threshold specified when not allowed."""
    pass
...
class Throttle:
    ...
    def send_request(self,
        ...
        ################################################################
            ...
            # For straight sync and sync with leaky bucket, the
            # _early_arrival_count will have no effect. For straight
            ...
            # makes sense. Putting a try around the call here and
            # raising an exception might not be what the caller wants.
    ...
    def schedule_requests(self) -> None:
        ...
        Raises:
            ...
            AsyncQIsNoneDuringShutdown: The Throttle schedule_requests
                method detected that the async queue does not exist.
            ...
            while True:
                ...
                if self.do_shutdown != Throttle.TYPE_SHUTDOWN_NONE:
                    if self.async_q is None:
                        raise AsyncQIsNoneDuringShutdown(
                            'The Throttle schedule_requests method '
                            'detected that the async queue does not'
                            'exist.'
                        )
